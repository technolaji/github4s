...
object AuthOps {
  ...
  trait Implicits {
    implicit def AuthOpsMHandler[M[_]](implicit M: Monad[M]): AuthOpsM.Handler[M] =
      new AuthOpsM.Handler[M] {
        ...
        def getAccessToken(
          ...
          data =
            NewOAuthRequest(client_id, client_secret, code, redirect_uri, state).asJson.noSpaces
        )
