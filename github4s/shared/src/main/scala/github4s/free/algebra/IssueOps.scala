...
object IssueOps {
  ...
  trait Implicits {
    implicit def IssueOpsMHandler[M[_]](implicit M: Monad[M]): IssueOpsM.Handler[M] =
      new IssueOpsM.Handler[M] {
        ...
        def list(config: Config, owner: String, repo: String): M[GHResponse[List[Issue]]] =
          httpClient
            .get[List[Issue]](config.accessToken, s"repos/$owner/$repo/issues", config.headers)
        ...
        def search(
          ...
          val queryString =
            s"${URLEncoder.encode(query)}+${searchParams.map(_.value).mkString("+")}"
          httpClient
        ...
        def edit(
          ...
          httpClient.patch[Issue](
            ...
            config.headers,
            data =
              EditIssueRequest(state, title, body, milestone, labels, assignees).asJson.noSpaces
          )
