...
object AuthInterpreter[C, M[_]](
  ...
  implicit val authHandler : AuthOps.Handler = new AuthOps.Handler {
  /**
  ...
  headers = Map ("Authorization" → s"Basic ${
  s"$username:$password".getBytes.toBase64
}") ++ headers,
